...
const listkey = ["mohramp3", "NYXTODGANZ"];
...
var len = 15
        ...
        var randomTextNumber = random+randomlagi+'---------Ramdansibadut'+'MOHRA--GANS';
...
router.get('/addapikey', (req, res, next) => {
    ...
    if (apikey != 'mohramp3') return res.json(loghandler.invalidKey)
...
router.get('/cewe/vietnam', async (req, res, next) => {
        var apikeyInput = req.query.apikey
	if(!apikeyInput) return res.json(loghandler.notparam)	
	if (apikeyInput != 'mohramp3')  return res.json(loghandler.invalidKey)
